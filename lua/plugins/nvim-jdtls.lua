return {

  {
    ...
    opts = function()
      ...
      local LazyUtils = require 'utils.utils'
      return {
    ...
    config = function(_, opts)
      ...
      local function attach_jdtls()
        ...
        -- Pega o root_dir dinamicamente, agora que jdtls está carregado
        local root_dir = lspconfig.jdtls.document_config.default_config.root_dir(fname)
        local project_name = opts.project_name(root_dir)

        -- Monta o comando base
        local cmd = { vim.fn.exepath 'jdtls' }

        -- Adiciona lombok.jar dinamicamente se o Mason estiver presente
        if LazyUtils.has 'mason.nvim' then
          local mason_registry = require 'mason-registry'
        ...
        -- Monta o comando completo com config e workspace
        if project_name then
          vim.list_extend(cmd, {
            '-configuration',
            opts.jdtls_config_dir(project_name),
